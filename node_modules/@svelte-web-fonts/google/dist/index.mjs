...
/* src/lib/GoogleFont.svelte generated by Svelte v3.48.0 */

const get_default_slot_changes = dirty => ({
	styleSheetUrl: dirty & /*styleSheetUrl*/ 1
});

const get_default_slot_context = ctx => ({ styleSheetUrl: /*styleSheetUrl*/ ctx[0] });
...
function fallback_block(ctx) {
	...
	return {
		c() {
			...
			attr(link, "href", /*styleSheetUrl*/ ctx[0]);
		},
		m(target, anchor) {
			insert(target, link, anchor);
		},
		p(ctx, dirty) {
			if (dirty & /*styleSheetUrl*/ 1) {
				attr(link, "href", /*styleSheetUrl*/ ctx[0]);
			}
		},
		d(detaching) {
...
function create_fragment(ctx) {
	...
	return {
		...
		p(ctx, [dirty]) {
			if (default_slot) {
				if (default_slot.p && (!current || dirty & /*$$scope, styleSheetUrl*/ 129)) {
					update_slot_base(
						default_slot,
				...
				if (default_slot_or_fallback && default_slot_or_fallback.p && (!current || dirty & /*styleSheetUrl*/ 1)) {
...
function instance($$self, $$props, $$invalidate) {
	...
	let { font = null } = $$props;
	let { fonts = null } = $$props;
	let { display = "auto" } = $$props;
	let { effect = "none" } = $$props;
	let { subsets = [] } = $$props;
	let { text = "" } = $$props;

	$$self.$$set = $$props => {
		if ('font' in $$props) $$invalidate(1, font = $$props.font);
		if ('fonts' in $$props) $$invalidate(2, fonts = $$props.fonts);
		if ('display' in $$props) $$invalidate(3, display = $$props.display);
		if ('effect' in $$props) $$invalidate(4, effect = $$props.effect);
		if ('subsets' in $$props) $$invalidate(5, subsets = $$props.subsets);
		if ('text' in $$props) $$invalidate(6, text = $$props.text);
		if ('$$scope' in $$props) $$invalidate(7, $$scope = $$props.$$scope);
	};

	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*font, fonts, display, subsets, effect, text*/ 126) {
			$$invalidate(0, styleSheetUrl = GoogleModule.getStylesheet(font !== null && font !== void 0 ? font : fonts, { display, subsets, effect, text }));
		}
	};

	return [styleSheetUrl, font, fonts, display, effect, subsets, text, $$scope, slots];
}

class GoogleFont extends SvelteComponent {
	constructor(options) {
		super();
